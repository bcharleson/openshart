...
2. If `"openclaw.extensions"` exists, it resolves entries from package root.
...
`openclaw.extensions` must be present, otherwise the manifest lookup happens in the wrong directory.
...
- Missing `openclaw.extensions`: OpenClaw scans subdirectories and looks for `openclaw.plugin.json` in `src/` or `dist/`.
